---
title: "Tidyverse: iteration and model"
author: "Advanced R"
...
1. Methods to iterate over lists.

2. Methods to tidy up model objects.

3. Construct consistent workflow for data wrangling and statistical modeling in the tidyverse.
...
* **Task 1:** model-based summary of feature log-intensities for all proteins in every run.
...
* **Task 2:** hypothesis testing to find differentially abundant proteins.
...
## Task 1: summarize feature log-intensities per protein with a linear model
...
tibble(effect = names(fit_coef), estimate = fit_coef) %>% 
    filter(str_detect(effect, "run")) %>% 
    mutate(effect = str_replace(effect, "run", ""))
```
...
1. The output objects of the modeling are not tidy. Additional efforts are required to extract and reformat (tidy) summaries of interest, where different summaries may need different utility function to obtain.
...
## Why a model object is hard to work with?
...
* It requires **multiple steps** to convert coefficients in a data frame with information restored in **row names** and its **column names** can be inconvenient to access.
* Some information (e.g., $p$-value) is computed in `print` method, but **not restored**.
...
* `broom::tidy()`: to extract **parameter-level** summaries
    + model coefficients, statistic
* `broom::augment()`: to extract **observation-level** summaries
    + predictions, residuals, raw values
...
Use broom to tidy up model objects:
...
We learned from the previous session how to obtain grouped summaries with arbitrary operations by using `dplyr::group_by()` + `dplyr::do()`. We can now carry out the task with these tools.


### Approach 3 for Task 1
...
twin_dia2 %>% 
    ...
    filter(str_detect(term, "run")) %>% 
    mutate(term = str_replace(term, "run", ""))
```
...
As discussed in the previous session, we can also save the fitted model for each protein in a new list-column: 
...
It's handy to get different levels of summaries by applying different tidying methods: 
...
Nested data frame organizes the data in a structurally-informative format: 

* Each **row** forms a group (e.g., protein).
* The `data` **column** stores the stratified data corresponding to each protein as a list (list-column).
...
Compared to the previous approaches, Approach 4 with nested data frame allows to explore the data in a principled way.


## Map over multiple arguments

* Use `purrr::map2(X, Y, FUN)` to map a function over two vectors or lists in parallel. 

* Use `purrr::pmap(LIST, FUN)` to map a function over two or more vectors or lists in parallel.

```{r}
# Use fitted model to make prediction: `predict(fit, data)`
nested_dia %>% 
...
# Same as
nested_dia %>% 
    mutate(pred = pmap(list(fit, data), predict))
```
